...
export class ConferenceCrawlJobController {
    ...
    async getUpdateStatus(
        ...
        return this.conferenceCrawlJobService.getUpdateStatus(page, perPage, status);
    }
    ...
    @ApiBody({
        schema: {
            ...
            properties: {
                ...
                batchSize: { type: 'number', description: 'Number of conferences to update per batch' }
            }
        }
    })
    async scheduleCronUpdate(
        ...
        return this.conferenceCrawlJobService.scheduleCronUpdate(schedule, batchSize);
    }
    ...
    async updateConference(@Param('id') id: string) {
        ...
        // Create update job
        const jobInstance = await this.conferenceCrawlJobService.createConferenceCrawlJob({
        ...
        return {
            ...
            conferenceId: conference.id,
            channel: `conference-crawl-job-${jobInstance.id}`
        };
    }
    ...
    async scheduleUpdate(@Query('batchSize') batchSize: number = 10) {
        ...
        for (let i = 0; i < conferences.length; i += batchSize) {
            ...
            const validBatchInputs = batchInputs.filter((input): input is ConferenceCrawlJobInputDTO => input !== null);
        ...
        for (const batch of batches) {
            const batchResult = await this.conferenceCrawlJobService.createBatchUpdateConferenceCrawlJob(batch);
