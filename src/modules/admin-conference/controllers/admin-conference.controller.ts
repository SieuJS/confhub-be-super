...
export class AdminConferenceController {
  ...
  async crawlNewConference(@Body() conferenceIds: string[]) {
    const conferences = await Promise.all(
      conferenceIds.map((id) => {
        return this.adminConferenceService
    ...
    const formData = toFormData(sendToCrawl)

    const response = await axios.post(
      'http://localhost:3001/crawl-conferences' + (new URLSearchParams(params)).toString(),
        formData
    );

    const data = response.data
    console.log('response', data)
    const parsedData = await this.adminConferenceService.parsePartEvaluateCsv(data);

    const imports = parsedData.map(async (item) => {
        return this.adminConferenceService.importEvaluateConference(item);
      });
